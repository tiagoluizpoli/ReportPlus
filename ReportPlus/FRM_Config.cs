...
namespace ReportPlus
{
    public partial class FRM_Config : Form
    {
        ...
        #region DeclaracaoVariaveisDeAmbiente
        ...
        private void btnSalvar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (bgWorkerConnectionTest.IsBusy != true)
                {
                    bgWorkerConnectionTest.RunWorkerAsync(pgrspnTesteConexao);
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnTestarConexao_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!bgWorkerConnectionTest.IsBusy)
                {
                    bgWorkerConnectionTest.RunWorkerAsync(pgrspnTesteConexao);
                }
                else
                {
                    MetroFramework.MetroMessageBox.Show(this, "Aguarde o termino da Consulta", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Warning, 150);
                }                
            }
            catch (Exception ex)
            {
        ...
        private void bgWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                bgWorkerConnectionTest.ReportProgress(1);
                Thread.Sleep(1000);
        ...
        private void btnClearPicLogoPath_Click(object sender, EventArgs e)
        {
            try
            {
                Win_Registry.gravar_pic_path("C:/");
                pbLogoImg.ImageLocation = string.Empty;
                txtbxPicLogoPath.Text = string.Empty;
            }
            catch (Exception ex)
            {
                MetroFramework.MetroMessageBox.Show(this, ex.Message, "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error, 150);
